...
app.get("/login", function (req, res) {
  ...
  const sql6 = "select * from cody where codyuserid =?;";
...
app.post("/login", (req, res) => {
  ...
  const sql8 = "select * from cody where codyuserid =?;";

  con.query(sql, [id], function (err, rows, fields) {
    if (rows.length) {
      if (rows[0].id == id) {
        con.query(
          sql2 + sql3 + sql4 + sql5 + sql6 + sql7 + sql8,
          [pw, id, id, id, id, id, id],
          function (err, rows, fields) {
            if (err) {
              throw err;
            }
      ...
      res.send(
        "<script>alert('아이디가 잘못되었습니다'); window.location.replace('/login');</script>"
      );
...
app.get("/codyselect", (req, res) => {
  ...
  const sql =
    "SELECT cody.*,shirt.*,pants.*,shoes.*,etc.*,overcoat.* FROM closet.cody left outer join shirt on cody.shirt= shirt.id left outer join pants on cody.pants=pants.id left outer join shoes on cody.shoes=shoes.id left outer join etc on cody.etc=etc.id left outer join overcoat on cody.overcoat=overcoat.id where cody_id =?;";
...
app.get("/codyregister", (req, res) => {
  ...
  const sql =
    "SELECT cody.*,shirt.*,pants.*,shoes.*,etc.*,overcoat.* FROM closet.cody left outer join shirt on cody.shirt= shirt.id left outer join pants on cody.pants=pants.id left outer join shoes on cody.shoes=shoes.id left outer join etc on cody.etc=etc.id left outer join overcoat on cody.overcoat=overcoat.id where codyuserid =? and codyimage is null;";
  const sql2 =
    "SELECT cody.*,shirt.*,pants.*,shoes.*,etc.*,overcoat.* FROM closet.cody left outer join shirt on cody.shirt= shirt.id left outer join pants on cody.pants=pants.id left outer join shoes on cody.shoes=shoes.id left outer join etc on cody.etc=etc.id left outer join overcoat on cody.overcoat=overcoat.id where codyuserid =? and codyimage is not null;";
  const sql3 = "select * from shirt where shuserid =?;";
  const sql4 = "select * from pants where puserid =?;";
  const sql5 = "select * from shoes where suserid =?;";
  const sql6 = "select * from etc where userid =?;";
  const sql7 = "select * from overcoat where ovuserid =?;";
  const sql8 = "select * from cody where codyuserid =?;";
  con.query(
    sql + sql2 + sql3 + sql4 + sql5 + sql6 + sql7 + sql8,
    [req.session.uid, req.session.uid, id, id, id, id, id, id],
...
app.get("/productdetail", (req, res) => {
  ...
  const sql2 = "SELECT * from comment where product_id=?;";
  const sql3 = "select * from shirt where shuserid =?;";
  const sql4 = "select * from pants where puserid =?;";
  const sql5 = "select * from shoes where suserid =?;";
  const sql6 = "select * from etc where userid =?;";
...
app.get("/productselect", (req, res) => {
  ...
  const sql8 = "select * from cody where codyuserid =?;";
  con.query(
    sql + sql3 + sql4 + sql5 + sql6 + sql7 + sql8,
    [id, id, id, id, id, id],
    function (err, result, fields) {
      if (err) throw err;
      console.log(result);
...
app.get("/myshop", (req, res) => {
  ...
  const sql = "SELECT * from product where prouserid =?;";

  con.query(sql, [pid], function (err, result, fields) {
    if (err) throw err;
    console.log(result);
    res.render("myshop", { product: result });
  });
});
...
app.post("/product", (req, res) => {
  ...
  console.log(title);
  const sql =
    "INSERT INTO product (name, region, price,description,product_img,product_img2,product_img3,product_img4,product_img5,product_oneimage,prouserid) VALUES(?,?,?,?,?,?,?,?,?,?,?);";
  con.query(
    sql,
    [
      title,
...
app.get("/product", (req, res) => {
  ...
  const dsd = "";
  const sql =
    "SELECT shimage,pimage,simage,ovimage,image FROM closet.cody left outer join shirt on cody.shirt= shirt.id left outer join pants on cody.pants=pants.id left outer join shoes on cody.shoes=shoes.id left outer join etc on cody.etc=etc.id left outer join overcoat on cody.overcoat=overcoat.id where cody_id =?;";
...
app.post("/comment", (req, res) => {
  ...
  const sql =
    "INSERT INTO comment (product_id, conuserid, content) VALUES(?,?,?);";
  con.query(sql, [pid, uid, cont], function (err, result, fields) {
    if (err) throw err;
    res.redirect("back");
  });
});
app.post("/codypicregister", upload.single("codyimage"), (req, res) => {
  ...
  const sql =
    "INSERT INTO cody (codyname,shirt, pants,shoes,overcoat,etc,codyspec,seasonspec,codyuserid,codyimage) VALUES(?,?,?,?,?,?,?,?,?,?);";
  con.query(
    sql,
    [
      codyname,
...
app.post("/codyregister", upload.single("codyimage"), (req, res) => {
  ...
  const sql =
    "INSERT INTO cody (codyname,shirt, pants,shoes,overcoat,etc,codyspec,seasonspec,codyuserid) VALUES(?,?,?,?,?,?,?,?,?);";
  con.query(
    sql,
    [
      codyname,
...
app.post("/", upload.single("testimage"), (req, res) => {
  ...
  if (spec == "하의") {
    const sql =
      "INSERT INTO pants (pname, psize, pcloset_spec,pimage,puserid,pprice,pdry) VALUES(?,?,?,?,?,?,?);";
    con.query(
      sql,
      [testnum, size, closet, imagetest, userid, price, dry],
      function (err, result, fields) {
        if (err) throw err;
        res.redirect("/login");
      }
    ...
    const sql =
      "INSERT INTO shirt (shname, shsize, shcloset_spec,shimage,shuserid,shprice,shdry) VALUES(?,?,?,?,?,?,?);";
    con.query(
      sql,
      [testnum, size, closet, imagetest, userid, price, dry],
      function (err, result, fields) {
    ...
    const sql =
      "INSERT INTO shoes (sname, ssize, scloset_spec,simage,suserid,sprice,sdry) VALUES(?,?,?,?,?,?,?);";
    con.query(
      sql,
      [testnum, size, closet, imagetest, userid, price, dry],
      function (err, result, fields) {
    ...
    const sql =
      "INSERT INTO overcoat (ovname, ovsize, ovcloset_spec,ovimage,ovuserid,ovprice,ovdry) VALUES(?,?,?,?,?,?,?);";
    con.query(
      sql,
      [testnum, size, closet, imagetest, userid, price, dry],
      function (err, result, fields) {
    ...
    const sql =
      "INSERT INTO etc (name, size, closet_spec,image,userid,price,dry) VALUES(?,?,?,?,?,?,?);";
    con.query(
      sql,
      [testnum, size, closet, imagetest, userid, price, dry],
      function (err, result, fields) {
...
app.get("/delete/:id", (req, res) => {
  if (req.query.spec == "상의") {
    ...
    const sql = "DELETE FROM overcoat WHERE id = ?";
    con.query(sql, [req.params.id], function (err, result, fields) {
      if (err) throw err;
      console.log(result);
      res.redirect("/login");
...
app.get("/codydelete/:id", (req, res) => {
  const sql = "DELETE FROM cody WHERE cody_id = ?";
  con.query(sql, [req.params.id], function (err, result, fields) {
    if (err) throw err;
    console.log(result);
    res.redirect("/codyregister");
...
app.get("/fall", (req, res) => res.sendFile(path.join(__dirname, "fall.html")));
